	...
	for (i in 1:nrow(spp_list)){
		spp_card_list[[i]] <- card(
		              ...
		              card_image(file = paste0('./www/',
		                                       ...
		                                       ".png"),
		              			height = '150px'),
		              actionButton(inputId = paste0(spp_list$Species[i], '_btn'), 
		                           label = h5('Details'),
		                           width = '200px',
		...
		spp_tab_list[[i]] <- nav_panel(title = h4(paste(spp_list$Species[i], 'Rockfish')),	card(
		                 p("Species details"),
		                 actionButton(inputId = paste0(spp_list$Species[i], '_btnclose'), label = h5('Close'),
		                              width = '200px',
		                              style = 'background:black; color:white'),
		                 style = 'width: 300 px'),
						value = paste0(spp_list$Species[i], '_tab')
						)
